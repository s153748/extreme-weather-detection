...
def generate_geo_map(geo_data, month_select, graph_select, style_select):
    ...
    if graph_select == 'Point map':
        fig = px.scatter_mapbox(filtered_data, 
                                ...
                                color_discrete_sequence=['#a5d8e6'] if style_select=='darkmatter' else ['#457582'])
    else:
        fig = ff.create_hexbin_mapbox(data_frame=filtered_data, 
                                      ...
                                      original_data_marker=dict(size=5, opacity=1, color='#a5d8e6' if style_select=='darkmatter' else '#457582'))
